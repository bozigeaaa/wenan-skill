...
This skill is for single-point sanitary product logic. It should explain one concrete question about sanitation, compliance, maintenance, deployment, or selection, then connect Eastern Camel's product or delivery capability only when it naturally answers that question.
...
- Why campsite sanitary units are not a small accessory in Saudi projects.
...
One video explains one sanitary-unit point. Do not turn one script into a full K-series catalog.

Rewrite broad topics into one precise question:

- "Why do some mobile toilets smell after use?"
- "Which K-series configuration fits this camp scene?"
- "Why does packing size matter for urgent Saudi site deployment?"
- "Who is actually under pressure when camp sanitation fails?"
- "Which parameter answers the customer's maintenance concern?"
...
4. Build the script.
   ...
   - Default structure:
     1. Point out one real but often delayed sanitary problem.
     2. Explain the mechanism in plain Chinese.
     3. Translate it into a Saudi project pressure scene.
     4. Land TBOX / K-series / Eastern Camel capability only if relevant.
...
- Explain product terms immediately in plain Chinese.
- Prefer "卫生间不是最后才补的小配套", "真正承压的不是工人，是项目经理", "臭味问题通常不是一个通风口能解决", "型号不是越大越好，要看使用场景和维护边界".
